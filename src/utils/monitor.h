...
typedef struct {
    ...
    char mask_read_write_execute[FLAGS_MAX];
    char mask_create_delete_move[FLAGS_MAX];
...
monitor_box_t* init_monitor_box(char* parent_path, char* mount_path, 
    ...
    memset(m_box->fanotify_info.mask_create_delete_move, 0, sizeof(m_box->fanotify_info.mask_create_delete_move));
    memset(m_box->fanotify_info.mask_read_write_execute, 0, sizeof(m_box->fanotify_info.mask_read_write_execute));
    ...
    strncpy(m_box->fanotify_info.flags_fanotify_mark, "FAN_MARK_ADD, FAN_MARK_FILESYSTEM", strlen("FAN_MARK_ADD, FAN_MARK_FILESYSTEM") + 1);
    #else
    strncpy(m_box->fanotify_info.flags_fanotify_mark, "FAN_MARK_ADD, FAN_MARK_MOUNT", strlen("FAN_MARK_ADD, FAN_MARK_MOUNT")  + 1);
    #endif
    ...
    if (m_box->fanotify_info.config_fanotify_enabled) {
        ...
        strncat(m_box->fanotify_info.mask_read_write_execute, "FAN_EVENT_ON_CHILD, ", strlen("FAN_EVENT_ON_CHILD, ") + 1);
        #endif

        #ifdef FAN_ONDIR
        m_box->fanotify_info.event_mask_read_write_execute |= FAN_ONDIR;  
        strncat(m_box->fanotify_info.mask_read_write_execute, "FAN_ONDIR, ", strlen("FAN_ONDIR, ") + 1);
        #endif

        #ifdef FAN_ACCESS
        m_box->fanotify_info.event_mask_read_write_execute |= FAN_ACCESS;
        strncat(m_box->fanotify_info.mask_read_write_execute, "FAN_ACCESS, ", strlen("FAN_ACCESS, ") + 1);
        #endif

        #ifdef FAN_OPEN
        m_box->fanotify_info.event_mask_read_write_execute |= FAN_OPEN;
        strncat(m_box->fanotify_info.mask_read_write_execute, "FAN_OPEN, ", strlen("FAN_OPEN, ") + 1);
        #endif

        #ifdef FAN_MODIFY
        m_box->fanotify_info.event_mask_read_write_execute |= FAN_MODIFY;
        strncat(m_box->fanotify_info.mask_read_write_execute, "FAN_MODIFY, ", strlen("FAN_MODIFY, ") + 1);
        #endif

        #ifdef FAN_OPEN_EXEC
        m_box->fanotify_info.event_mask_read_write_execute |= FAN_OPEN_EXEC;
        strncat(m_box->fanotify_info.mask_read_write_execute, "FAN_OPEN_EXEC, ", strlen("FAN_OPEN_EXEC, ") + 1);
        #endif

        #ifdef FAN_CLOSE_WRITE
        m_box->fanotify_info.event_mask_read_write_execute |= FAN_CLOSE_WRITE;
        strncat(m_box->fanotify_info.mask_read_write_execute, "FAN_CLOSE_WRITE, ", strlen("FAN_CLOSE_WRITE, ") + 1);
        #endif
        ...
        strncat(m_box->fanotify_info.mask_create_delete_move, "FAN_ONDIR, ", strlen("FAN_ONDIR, ") + 1);
        #endif

        #ifdef FAN_CREATE
        m_box->fanotify_info.event_mask_create_delete_move |= FAN_CREATE;
        strncat(m_box->fanotify_info.mask_create_delete_move, "FAN_CREATE, ", strlen("FAN_CREATE, ") + 1);
        #endif
        
        #ifdef FAN_DELETE
        m_box->fanotify_info.event_mask_create_delete_move |= FAN_DELETE;
        strncat(m_box->fanotify_info.mask_create_delete_move, "FAN_DELETE, ", strlen("FAN_DELETE, ") + 1);
        #endif

        #ifdef FAN_RENAME 
        m_box->fanotify_info.event_mask_create_delete_move |= FAN_RENAME;
        strncat(m_box->fanotify_info.mask_create_delete_move, "FAN_RENAME, ", strlen("FAN_RENAME, ") + 1);
        #endif 

        #ifdef FAN_MOVED_FROM
        m_box->fanotify_info.event_mask_create_delete_move |= FAN_MOVED_FROM;
        strncat(m_box->fanotify_info.mask_create_delete_move, "FAN_MOVED_FROM, ", strlen("FAN_MOVED_FROM, ") + 1);
        #endif

        #ifdef FAN_MOVED_TO
        m_box->fanotify_info.event_mask_create_delete_move |=FAN_MOVED_TO;
        strncat(m_box->fanotify_info.mask_create_delete_move, "FAN_MOVED_TO, ", strlen("FAN_MOVED_TO, ") + 1);
        #endif

        m_box->fanotify_info.mask_create_delete_move[strlen(m_box->fanotify_info.mask_create_delete_move) - 2] = '\0';
    ...
    if (buflen > 0) {
        ...
        while (FAN_EVENT_OK(metadata, buflen)) {
            ...
            if (metadata->mask & FAN_ONDIR) {
                strncat(mask, "FAN_ONDIR, ", strlen("FAN_ONDIR, ") + 1);
            }
            #endif

            #ifdef FAN_ACCESS
            if (metadata->mask & FAN_ACCESS) {
                strncat(mask, "FAN_ACCESS, ", strlen("FAN_ACCESS, ") + 1);
            }
            #endif

            #ifdef FAN_OPEN
            if (metadata->mask & FAN_OPEN) {
                strncat(mask, "FAN_OPEN, ", strlen("FAN_OPEN, ") + 1);
            }
            #endif 

            #ifdef FAN_MODIFY
            if (metadata->mask & FAN_MODIFY) {
                strncat(mask, "FAN_MODIFY, ", strlen("FAN_MODIFY, ") + 1);
            }
            #endif

            #ifdef FAN_OPEN_EXEC
            if (metadata->mask & FAN_OPEN_EXEC) {
                strncat(mask, "FAN_OPEN_EXEC, ", strlen("FAN_OPEN_EXEC, ") + 1);
            }
            #endif

            #ifdef FAN_CLOSE_WRITE
            if (metadata->mask & FAN_CLOSE_WRITE) {
                strncat(mask, "FAN_CLOSE_WRITE, ", strlen("FAN_CLOSE_WRITE, ") + 1);
            }
            #endif

            #ifdef FAN_CLOSE_NOWRITE
            if (metadata->mask & FAN_CLOSE_NOWRITE) {
                strncat(mask, "FAN_CLOSE_NOWRITE, ", strlen("FAN_CLOSE_NOWRITE, ") + 1);
            }
            #endif
            ...
            mask[strlen(mask) - 2] = '\0';
            log_message(INFO, 1, "%s (%d): %s == [%s]\n", comm, metadata->pid, full_path, mask);
            memset(mask, 0, sizeof(mask));
    ...
    if (buflen > 0) {
        ...
        while (FAN_EVENT_OK(metadata, buflen)) {
            ...
            if (metadata->mask & FAN_CREATE) {
                strncat(mask, "FAN_CREATE, ", strlen("FAN_CREATE, ") + 1);
            }
            #endif

            #ifdef FAN_DELETE
            if (metadata->mask & FAN_DELETE) {
                strncat(mask, "FAN_DELETE, ", strlen("FAN_DELETE, ") + 1);
            }
            #endif

            #ifdef FAN_RENAME
            if (metadata->mask & FAN_RENAME) {
                strncat(mask, "FAN_RENAME, ", strlen("FAN_RENAME, ") + 1);
            }
            #endif

            #ifdef FAN_MOVED_FROM
            if (metadata->mask & FAN_MOVED_FROM) {
                strncat(mask, "FAN_MOVED_FROM, ", strlen("FAN_MOVED_FROM, ") + 1);
            }
            #endif

            #ifdef FAN_MOVED_TO
            if (metadata->mask & FAN_MOVED_TO) {
                strncat(mask, "FAN_MOVED_TO, ", strlen("FAN_MOVED_TO, ") + 1);
            }
            #endif
            ...
            mask[strlen(mask) - 2] = '\0';
            if (file_name) {
                log_message(INFO, 1, "%s (%d): %s/%s == [%s]\n", comm, metadata->pid, path, file_name, mask);
            } else {
                log_message(INFO, 1, "%s (%d): %s == [%s]\n", comm, metadata->pid, path, mask);
            }
            memset(mask, 0, sizeof(mask));
...
void print_box(monitor_box_t* m_box) {
    ...
    log_message(NIL, 0, "\t└─ Flag: %s\n", m_box->fanotify_info.flags_fanotify_mark);
    log_message(NIL, 0, "\t└─ Mask: %s\n", m_box->fanotify_info.mask_read_write_execute);
    log_message(NIL, 0, "- Fanotify Create, Delete, Move FD: %d\n", m_box->fanotify_info.fd_create_delete_move);
    log_message(NIL, 0, "\t└─ Flag: %s\n", m_box->fanotify_info.flags_fanotify_mark);
    log_message(NIL, 0, "\t└─ Mask: %s\n\n", m_box->fanotify_info.mask_create_delete_move);
